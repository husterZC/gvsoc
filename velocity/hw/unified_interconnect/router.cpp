...
private:
    ...
    static vp::IoReqStatus top_slave_req(vp::Block *__this, vp::IoReq *req);
    ...
    void forward_req(vp::IoReq *req, int rid);
    void process_collective_req();

    vp::Trace                               trace;
    std::vector<vp::IoSlave>                input_itf;
    std::vector<vp::IoMaster>               output_itf;
    vp::IoMaster                            top_master_ift;
    vp::IoSlave                             top_slave_ift;
    ...
    std::vector<vp::IoReq *>                collective_req_list;
};
    ...
    : vp::Component(config)
{
    ...
    this->collective_req_list.resize(this->num_port);
    this->new_master_port("top_req_master", &this->top_master_ift);
    this->new_slave_port("top_req_slave", &this->top_slave_ift);
    this->top_slave_ift.set_req_meth(&Router::top_slave_req);
...
vp::IoReqStatus Router::top_slave_req(vp::Block *__this, vp::IoReq *req)
{
    Router *_this = (Router *)__this;
    
    //Check if RX FIFO is full
    if (_this->collective_req_list.size() >= _this->num_port) {
        _this->trace.msg(vp::Trace::LEVEL_TRACE, "[Router] Collective FIFO is full, reject the request\n");
        *req->arg_get(UnifiedInterconnect::REQ_ACK) = (void *)UnifiedInterconnect::ACK_COLLECTIVE_REJECT;
    } else {
        _this->trace.msg(vp::Trace::LEVEL_TRACE, "[Router] Collective FIFO Accept the request, put it into RX FIFO\n");
        *req->arg_get(UnifiedInterconnect::REQ_ACK) = (void *)UnifiedInterconnect::ACK_COLLECTIVE_ACCEPT;
        int rid = *(int *)req->arg_get(UnifiedInterconnect::REQ_PORT_ID);
        vp::IoReq * tmp_req = _this->new_req(req, rid);
        _this->collective_req_list.push_back(tmp_req);
...
{
    int source_port = *(int *)req->arg_get(UnifiedInterconnect::REQ_PORT_ID);
    vp::IoReq * tmp_req = this->new_req(req, source_port);
    *tmp_req->arg_get(UnifiedInterconnect::REQ_ACK) = (void *)0;
    vp::IoReqStatus result = this->top_master_ift.req(tmp_req);
    ...
    int destination_port = *(int *)tmp_req->arg_get(UnifiedInterconnect::REQ_PORT_ID);
    this->del_req(tmp_req);
    if (ack < 0)
    {
        return ack; // This means it is a collective request
    }
    return destination_port;
}

void Router::forward_req(vp::IoReq *req, int rid)
{
    //Set routing port
    *req->arg_get(UnifiedInterconnect::REQ_PORT_ID) = (void *)rid;
...
void Router::process_collective_req()
{
    int num_collective_req = this->collective_req_list.size();
    std::vector<vp::IoReq *> remain_req;
    for (int i = 0; i < num_collective_req; ++i)
    {
        vp::IoReq * req = this->collective_req_list[i];
        int rid = *(int *)req->arg_get(UnifiedInterconnect::REQ_PORT_ID);
        if (this->output_stall_list[rid] == 0)
        {
            this->forward_req(req, rid);
        } else {
            remain_req.push_back(req);
        }
    }
    this->collective_req_list = remain_req;
}
...
int Router::process_rx_port(int port)
{
    ...
    for (int i = 0; i < num_req; ++i)
    {
        ...
        if (rid == UnifiedInterconnect::ACK_COLLECTIVE_REJECT) {
            //The topology manager is not ready for processing collective requests, we need to retry later
            remain_req.push_back(req);
            continue;
        } else if (rid == UnifiedInterconnect::ACK_COLLECTIVE_ACCEPT) {
            //This is a collective request, and the topology manager recieved it successfully
            this->del_req(req);
            forwarded = 1;
            continue;
        }
...
void Router::routing_fsm_handler(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    //First process collective requests since they are latency sensitive
    _this->process_collective_req();
...
{
    Router *_this = (Router *)__this;
    int i = *(int *)req->arg_get(UnifiedInterconnect::REQ_PORT_ID);
